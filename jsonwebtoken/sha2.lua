...
local function pad(msg, block, lenbytes)
    ...
    return msg .. "\128" .. ("\0"):rep(zeros + lenbytes - 8)
        .. string.pack(">I8", bits)
end
...
function sha2.sha256(msg)
    ...
    for off = 1, #msg, 64 do
        ...
        h1, h2, h3, h4 = (h1 + a) & 0xffffffff, (h2 + b) & 0xffffffff,
            (h3 + c) & 0xffffffff, (h4 + d) & 0xffffffff
        h5, h6, h7, h8 = (h5 + e) & 0xffffffff, (h6 + f) & 0xffffffff,
            (h7 + g) & 0xffffffff, (h8 + h) & 0xffffffff
    end
...
local function sha512_core(msg, iv, outwords)
    ...
    return string.pack(">" .. ("i8"):rep(outwords),
        table.unpack(words, 1, outwords))
end
...
function sha2.hmac(alg, key, msg)
    local h = HASH[alg]
    if not h then
        error(("Unknown hash %q (sha256, sha384 or sha512)")
            :format(tostring(alg)), 2)
    end
    if #key > h.block then
        key = h.digest(key)
    end
